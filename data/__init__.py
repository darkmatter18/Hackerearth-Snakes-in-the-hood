...
def create_dataset(opt):
    ...
    train_dataset = SnakeDataset(opt.dataroot, opt.phase, train_data, opt.preprocess, True, True, opt.load_size,
                                 opt.crop_size)
    test_dataset = SnakeDataset(opt.dataroot, opt.phase, test_data, opt.preprocess, False, True, opt.load_size,
                                opt.crop_size)
...
def create_test_dataset(opt):
    ...
    test_dataset = SnakeDataset(opt.dataroot, opt.phase, data, opt.preprocess, False, False, opt.load_size,
                                opt.crop_size)
    testloader = SnakeDataLoader(test_dataset, opt.batch_size, opt.num_threads, shuffle=False)
